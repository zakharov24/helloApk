...
"${BUILD_TOOLS}/aapt" package -f -M AndroidManifest.xml -S res/  -I "${PLATFORM}/android.jar" \
		-F build/Voting.unsigned.apk build/apk/
"${BUILD_TOOLS}/zipalign" -f 4 build/Voting.unsigned.apk build/Voting.aligned.apk
apksigner sign --ks keystore.jks \
        --ks-key-alias androidkey --ks-pass pass:android \
		      --key-pass pass:android --out build/Voting.apk \
			  build/Voting.aligned.apk
